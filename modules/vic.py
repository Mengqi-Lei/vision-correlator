...
class DeepViC(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.stem = LKStem(out_dim=channels, act=act)
        ...
        if use_dilation:
            self.backbone = Seq(*[Seq(ViCBlock(channels,
                                                      kernel_size=num_knn[i],
                                                      dilation=min(i // 4 + 1, max_dilation),
                                                      act=act, norm=norm, bias=bias,
                                                      stochastic=stochastic, epsilon=epsilon, r=1, n=n,
                                                      drop_path=dpr[i], relative_pos=relative_pos,
                                                      num_hyperedges=num_hyperedges_list[i],
                                                      laplacian_alpha=laplacian_alpha,
                                                      hypergraph_k=num_knn[i],
...
@register_model
def vic_ti_224_gelu(pretrained=False, **kwargs):
    class OptInit:
...
@register_model
def vic_s_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, drop_rate=0.0,
                     k=9, num_knn=None, relative_pos=False, use_dilation=True, **kwargs):
            if num_knn is not None:
...
@register_model
def vic_b_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, drop_rate=0.0,
                     k=9, num_knn=None, relative_pos=False, use_dilation=True, **kwargs):
            if num_knn is not None:
